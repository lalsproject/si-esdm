...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `tbv_tambang`  AS SELECT `a`.`IdTambang` AS `IdTambang`, `b`.`IdPerusahaan` AS `IdPerusahaan`, `b`.`Perusahaan` AS `Perusahaan`, `b`.`AlamatPerusahaan` AS `AlamatPerusahaan`, `c`.`IdType` AS `IdType`, `c`.`Type` AS `Type`, `d`.`IdDaerah` AS `IdDaerah`, `d`.`Daerah` AS `Daerah`, `a`.`Lat` AS `Lat`, `a`.`Long` AS `Long`, `a`.`Lokasi` AS `Lokasi`, `a`.`Luas` AS `Luas`, `a`.`Tahapan` AS `Tahapan`, `a`.`SK` AS `SK`, `a`.`NoSK` AS `NoSK`, `a`.`TAwal` AS `TAwal`, `a`.`TSelesai` AS `TSelesai`, `a`.`Komoditas` AS `Komoditas`, `a`.`Status` AS `Status`, `a`.`Ket` AS `Ket`, `a`.`Foto` AS `Foto`, `a`.`RegDate` AS `RegDate` FROM (((`tbl_mas_tambang` `a` left join `tbl_mas_perusahaan` `b` on(`a`.`IdPerusahaan` = `b`.`IdPerusahaan`)) left join `tbl_mas_type_mineral` `c` on(`a`.`IdType` = `c`.`IdType`)) left join `tbl_mas_daerah` `d` on(`a`.`IdDaerah` = `d`.`IdDaerah`)) ORDER BY `a`.`RegDate` ASC  ;
